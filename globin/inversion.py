	...
	def invert_pxl_by_pxl(self):
		...
		while np.min(itter) <= self.max_iter:
			...
			if len(skip)!=(atmos.nx*atmos.ny):
				...
				elif self.rf_type=="node":
					# rf, spec, _ = globin.compute_rfs(atmos, skip, rf_noise_scale=noise_stokes)
					rf, spec, _ = self.atmosphere.compute_rfs(weights=self.weights, rf_noise_scale=noise_stokes, skip=skip, rf_type=self.rf_type)
